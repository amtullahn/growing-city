...
BEGIN_MESSAGE_MAP(CChildView, CWnd)
    ...
    ON_COMMAND(ID_LANDSCAPING_GARDENTILE, &CChildView::OnLandscapingGardenTile)
    ON_UPDATE_COMMAND_UI(ID_VIEW_MOUSEHISTORY32811, &CChildView::OnUpdateViewMouseHistory)
    ON_COMMAND(ID_VIEW_MOUSEHISTORY32811, &CChildView::OnViewMouseHistory)
    ON_COMMAND(ID_GARDEN_PRUNEALL32812, &CChildView::OnGardenPruneAll)
    ON_UPDATE_COMMAND_UI(ID_GARDEN_PRUNE32813, &CChildView::OnUpdateGardenPrune)
    ON_COMMAND(ID_GARDEN_PRUNE32813, &CChildView::OnGardenPrune)
END_MESSAGE_MAP()
/// \endcond
...
void CChildView::OnPaint()
{
    ...
    */
    ...
    // Draw a line between points to track mouse position
    if (mShowHistory)
    {
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (mGrabbedItem != nullptr)
    {
        ...
        if (mPruningActive == true) 
        {
            CGardenVisitor visitor;
...
void CChildView::OnBuildingsCount()
{
    ...
    //Sends it off to the city to each tile in turn
    //Each tile calls the appropriate function for that tile type
    mCity.Accept(&visitor);
    // For building the tiles, VisitBuilding is called incrementing the count
    int cnt = visitor.GetNumBuildings();

    wstringstream str;
    //We just get the count and display it.
    str << L"There are " << cnt << L" buildings.";
    AfxMessageBox(str.str().c_str());
}

/**
 * Adding an initial garden tile image
 */
void CChildView::OnLandscapingGardenTile()
{
    // TODO: Add your command handler code here
    OnAddGarden(L"garden.png");
}


/**
 * Adds a tile to the city and sets the location
 * \param file A tile that's passed in
 */
void CChildView::OnAddGarden(const std::wstring& file)
{
...
/**
 * Handler for updating a check on Mouse History 
 * \param pCmdUI Passes in the update command
 */
...
/**
 * Handler for Viewing Mouse History
 */
...
/**
 * Handle the Pruning All>Garden Option
 */
...
/**
 * Update the checkmark for the Garden>Pruning menu option
 * \param pCmdUI Passes in the update command
 */
void CChildView::OnUpdateGardenPrune(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(mPruningActive);
}

/**
 * Handle the Pruning>Garden Option
 */
void CChildView::OnGardenPrune()
{
    // TODO: Add your command handler code here
    mPruningActive = !mPruningActive;
}
